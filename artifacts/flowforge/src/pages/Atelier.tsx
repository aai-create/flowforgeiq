    ...
    <Dialog open={showNewPO} onOpenChange={open => { if (!open) resetNewPO(); }}>
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-4 py-1">
          ...
          <div>
            {(() => {
              const _milestoneTotal = milestones.reduce((s, m) => s + (Number(m.percent) || 0), 0);
              return (
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center gap-2">
                    <label className="block text-xs font-semibold text-[#5E687B]">{t("orders.paymentMilestones")}</label>
                    <span className={`text-xs font-semibold px-1.5 py-0.5 rounded-full ${_milestoneTotal > 100 ? "bg-red-100 text-red-700" : _milestoneTotal === 100 ? "bg-emerald-100 text-emerald-700" : "bg-amber-100 text-amber-700"}`}>
                      {_milestoneTotal}%
                    </span>
                  </div>
                  {milestones.length < 3 && (
            ...
            {(() => {
              ...
              if (total > 100) return (
                <p className="text-xs text-red-600 mt-1.5">{t("orders.percentagesSumExceed", { total })}</p>
              );
              if (total < 100) return (
        ...
        <DialogFooter className="gap-2">
          ...
          {editingShipmentId !== null ? (
            ...
            <button onClick={submitNewPO}
              disabled={createShipmentMutation.isPending || createSupplierMutation.isPending || milestones.reduce((s, m) => s + (Number(m.percent) || 0), 0) > 100}
              className="flex items-center gap-2 px-4 py-2 text-sm font-semibold text-white bg-[#9000FF] hover:bg-[#7A00D9] disabled:opacity-60 rounded-md transition-colors">
