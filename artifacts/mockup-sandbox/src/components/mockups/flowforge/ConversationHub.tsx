...
import {
  ...
  Paperclip, Send, ArrowRight, Home, Inbox, FileBox, Users, Filter,
...
function ViewSwitcher({ mode, setMode }: { mode: ViewMode; setMode: (m: ViewMode) => void }) {
  ...
  return (
    <div ref={ref} style={{ position:"fixed", left:pos.x, top:pos.y, zIndex:60, userSelect:"none" }}
      ...
      <div onMouseDown={onMouseDown} className="pl-2 pr-1 flex items-center cursor-grab active:cursor-grabbing text-[#C0C8D4] hover:text-[#9000FF] transition-colors">
...
function StageConfigModal({ stages, onSave, onClose }: { stages: Stage[]; onSave: (s: Stage[]) => void; onClose: () => void }) {
  ...
  const addStage = () => { const l = newLabel.trim(); if (!l) return; setDraft(d => [...d, { id:`custom_${Date.now()}`, label:l }]); setNewLabel(""); };
  const moveStage = (from: number, to: number) => { setDraft(d => { const n=[...d]; const [i]=n.splice(from,1); n.splice(to,0,i); return n; }); };
  return (
    <div className="fixed inset-0 z-[80] bg-black/30 flex items-center justify-center" onClick={e => { if (e.target===e.currentTarget) onClose(); }}>
      <div className="bg-white rounded-2xl shadow-[0_20px_60px_rgba(0,0,0,0.18)] w-[480px] max-h-[80vh] flex flex-col overflow-hidden border border-[#E5EAF0]">
        <div className="flex items-center justify-between px-5 py-4 border-b border-[#E5EAF0]">
          <div><div className="font-bold text-sm text-[#212833] flex items-center gap-2"><SlidersHorizontal size={15} className="text-[#9000FF]"/>Workflow Stages</div><div className="text-[10px] text-[#5E687B] mt-0.5">Drag to reorder · × to remove · add custom stages below</div></div>
          <button onClick={onClose} className="p-1.5 hover:bg-[#F0F4F8] rounded-full text-[#5E687B]"><X size={16}/></button>
        </div>
        <div className="flex-1 overflow-y-auto p-4 flex flex-col gap-1.5">
          {draft.map((stage, idx) => (
            <div key={stage.id} draggable onDragStart={() => setDragIdx(idx)} onDragOver={e => { e.preventDefault(); setOverIdx(idx); }} onDrop={() => { if (dragIdx!==null&&dragIdx!==idx) moveStage(dragIdx,idx); setDragIdx(null); setOverIdx(null); }} onDragEnd={() => { setDragIdx(null); setOverIdx(null); }}
              className={`flex items-center gap-3 px-3 py-2.5 rounded-lg border transition-all cursor-grab ${overIdx===idx?"border-[#9000FF]/40 bg-[#FAFBFF]":dragIdx===idx?"opacity-40 border-[#E5EAF0]":"border-[#E5EAF0] bg-white hover:border-[#D6E3EB]"}`}>
              ...
              <div className="flex gap-1">
                <button onClick={() => idx>0&&moveStage(idx,idx-1)} disabled={idx===0} className="p-1 hover:bg-[#F0F4F8] rounded text-[#5E687B] disabled:opacity-30"><ChevronUp size={12}/></button>
                <button onClick={() => idx<draft.length-1&&moveStage(idx,idx+1)} disabled={idx===draft.length-1} className="p-1 hover:bg-[#F0F4F8] rounded text-[#5E687B] disabled:opacity-30"><ChevronDown size={12}/></button>
                <button onClick={() => setDraft(d=>d.filter((_,i)=>i!==idx))} className="p-1 hover:bg-red-50 rounded text-[#C0C8D4] hover:text-red-500"><Trash2 size={12}/></button>
              </div>
            </div>
          ))}
        </div>
        <div className="px-4 py-3 border-t border-[#E5EAF0] flex gap-2">
          <input value={newLabel} onChange={e=>setNewLabel(e.target.value)} onKeyDown={e=>e.key==="Enter"&&addStage()} placeholder="Add a stage name..." className="flex-1 px-3 py-2 text-xs border border-[#E5EAF0] rounded-lg outline-none focus:border-[#9000FF]/40 focus:ring-1 focus:ring-[#9000FF]/10"/>
          <button onClick={addStage} disabled={!newLabel.trim()} className="px-3 py-2 bg-[#F0F4F8] text-[#212833] text-xs font-medium rounded-lg hover:bg-[#E5EAF0] disabled:opacity-40 flex items-center gap-1.5"><Plus size={13}/>Add</button>
        </div>
        <div className="px-4 py-3 border-t border-[#E5EAF0] flex justify-end gap-2">
          <button onClick={onClose} className="px-4 py-2 text-xs font-medium text-[#5E687B]">Cancel</button>
          <button onClick={() => { onSave(draft); onClose(); }} className="px-4 py-2 bg-[#9000FF] text-white text-xs font-semibold rounded-lg hover:bg-[#7A00D9] shadow-sm">Save Stages</button>
        </div>
      </div>
    </div>
  );
                  ...
                  <>
                    ...
                    <div className="flex flex-col gap-0.5">
                      {evts.slice(0,2).map((e,ei)=>(
                        <div key={ei} className={`text-[8px] font-semibold px-1 py-0.5 rounded border truncate ${eventColor(e)}`}>{e.po} {e.type==="payment"?"💳":"📦"}</div>
                      ))}
